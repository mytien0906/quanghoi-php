<?php
	...
	function save_static()
	{
		...
		if($id)
		{
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('photo',$data)) $func->redirect("index.php?com=photo&act=photo_static&type=".$type);
		...
		else
		{
			...
			{
				if($d->insert('photo',$data)) $func->redirect("index.php?com=photo&act=photo_static&type=".$type);
	...
	function save_photo()
	{
		...
		if($id)
		{
			...
			$d->where('id', $id);
			$d->where('type', $type);
			if($d->update('photo',$data)) $func->redirect("index.php?com=photo&act=man_photo&type=".$type."&p=".$curPage);
		...
		else
		{
			...
			if($numberPhoto && $dataMultiTemp)
			{
				...
				$func->redirect("index.php?com=photo&act=man_photo&type=".$type."&p=".$curPage);
			}
