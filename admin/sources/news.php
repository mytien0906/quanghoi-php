<?php
	...
	function update(){
		...
		die();
		$splistmenu = $d->rawQuery("select * from #_news1 where type ='dichvu' and hienthi > 0 order by id asc");

		foreach ($splistmenu as $key => $value) {
			/*$splist = $d->rawQueryOne("select * from #_news_list where type ='".$type."' and old_id=".$value['id_danhmuc']." and hienthi > 0 order by stt,id desc");
			if((int)$splist['id']>0){
	...
	function get_item()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man&type=".$type.$strUrl, false);

		$item = $d->rawQueryOne("select * from #_news where id = ? and type = ? limit 0,1",array($id,$type));
		 
 
		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man&type=".$type.$strUrl, false);
	...
	function save_item(){
		...
		$data_options = isset($_POST['options']) ? $_POST['options'] : null;
		...
		if($data){
			...
			if($data_options){
				$data['options']=json_encode($_POST['options'],JSON_UNESCAPED_UNICODE);
			} 
		...
		if($id && $act!='save_copy')
		{
			...
			if($d->update('news',$data))
			{
				...
				if($savehere) $func->redirect("index.php?com=news&act=edit&type=".$type.$strUrl."&id=".$id);
				else $func->redirect("index.php?com=news&act=man&type=".$type.$strUrl);
			}
			else
			{
				if($savehere) $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=edit&type=".$type.$strUrl."&id=".$id, false);
				else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man&type=".$type.$strUrl, false);
			}
		}
		else
		{
			...
			if($d->insert('news',$data))
			{
				...
				if($act=='save_copy')
				{
					if($savehere) $func->redirect("index.php?com=news&act=edit&type=".$type.$strUrl."&id=".$id_insert);
					else $func->redirect("index.php?com=news&act=man&type=".$type.$strUrl);
				}
				else
				{
			...
			else
			{
				if($act=='save_copy')
				{
					if($savehere) $func->transfer("Sao chép dữ liệu bị lỗi", "index.php?com=news&act=edit&type=".$type.$strUrl."&id=".$id_insert, false);
					else $func->transfer("Sao chép dữ liệu bị lỗi", "index.php?com=news&act=man&type=".$type.$strUrl, false);
				}
				else
				{
					if($savehere) $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=edit&type=".$type.$strUrl."&id=".$id_insert, false);
					else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man&type=".$type.$strUrl, false);
				}
			}
		}
	}
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			if($row['id'])
			{
				...
				$func->redirect( "index.php?com=news&act=man&type=".$type.$strUrl);
			}
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man&type=".$type.$strUrl, false);
		}
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man&type=".$type.$strUrl, false);
	}
	...
	function get_list()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);

		$item = $d->rawQueryOne("select * from #_news_list where id = ? and type = ? limit 0,1",array($id,$type));
		
		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
	...
	function save_list()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
		...
		if($id)
		{
			...
			if($d->update('news_list',$data))
			{
				...
				$func->redirect( "index.php?com=news&act=man_list&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
		}
	}
	...
	function delete_list()
	{
		...
		if($id)
		{
			...
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_list&type=".$type.$strUrl, false);
	}
	...
	function get_cat()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
		
		$item = $d->rawQueryOne("select * from #_news_cat where id = ? and type = ? limit 0,1",array($id,$type));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
	}
	...
	function save_cat()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
		...
		if($id)
		{
			...
			if($d->update('news_cat',$data))
			{
				...
				$func->redirect("index.php?com=news&act=man_cat&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
		}
	}
	...
	function delete_cat()
	{
		...
		if($id)
		{
			...
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_cat&type=".$type.$strUrl, false);
	}
	...
	function get_loai()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
		
		$item = $d->rawQueryOne("select * from #_news_item where id = ? and type = ? limit 0,1",array($id,$type));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
	}
	...
	function save_loai()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
		...
		if($id)
		{
			...
			if($d->update('news_item',$data))
			{
				...
				$func->redirect("index.php?com=news&act=man_item&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
		}
	}
	...
	function delete_loai()
	{
		...
		if($id)
		{
			...
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_item&type=".$type.$strUrl, false);
	}
	...
	function get_sub()
	{
		...
		if(!$id) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);

		$item = $d->rawQueryOne("select * from #_news_sub where id = ? and type = ? limit 0,1",array($id,$type));

		if(!$item['id']) $func->transfer("Dữ liệu không có thực", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
	}
	...
	function save_sub()
	{
		...
		if(empty($_POST)) $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
		...
		if($id)
		{
			...
			if($d->update('news_sub',$data))
			{
				...
				$func->redirect("index.php?com=news&act=man_sub&type=".$type.$strUrl);
			}
			else $func->transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
		}
		else
		{
			if(isset($_FILES['file']))
			...
			else $func->transfer("Lưu dữ liệu bị lỗi", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
		}
	}
	...
	function delete_sub()
	{
		...
		if($id)
		{
			...
			else $func->transfer("Xóa dữ liệu bị lỗi", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
		}
		elseif(isset($_GET['listid']))
		{
			$listid = explode(",",$_GET['listid']);
		...
		else $func->transfer("Không nhận được dữ liệu", "index.php?com=news&act=man_sub&type=".$type.$strUrl, false);
	}
?>